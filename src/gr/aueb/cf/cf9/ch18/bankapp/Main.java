...
import gr.aueb.cf.cf9.ch18.bankapp.service.AcountServiceImpl;
...
public class Main {
    ...
    private final static IAccountService accountService = new AcountServiceImpl(accountDAO);
            ...
            try{
                switch (option) {
                    case "1" -> {
                        ...
                        System.out.println("Παρακαλώ εισάγεται το υπόλοιπο");
                        ...
                        System.out.println("Ο λογαριασμός δημιουργήθηκε (ανανεώθηκε) επιτυχώς με iban: " + readOnlyDTO.iban() + "υπόλοιπο: "
                                + readOnlyDTO.balance());
                    }
                    case "2" -> {
                        System.out.print("Παρακαλώ εισάγεται το IBAN: ");
                        iban = scanner.nextLine().trim();
                        System.out.println("Παρακαλώ εισάγεται το ποσό κατάθεσης");
                        BigDecimal depositAmount =  new BigDecimal(scanner.nextLine().trim());

                        accountController.deposit(iban, depositAmount);
                        System.out.println("Επιτυχής κατάθεση: " + depositAmount + ", Νέο υπόλοιπο: " + accountController.getBalance(iban));
                    }
                    case "3" -> {
                        System.out.print("Παρακαλώ εισάγεται το IBAN: ");
                        iban = scanner.nextLine().trim();
                        System.out.println("Παρακαλώ εισάγεται το ποσό ανάληψης");
                        BigDecimal withdrawAmount =  new BigDecimal(scanner.nextLine().trim());

                        accountController.withdraw(iban, withdrawAmount);
                        System.out.println("Επιτυχής ανάληψη: " + withdrawAmount + ", Νέο υπόλοιπο: " + accountController.getBalance(iban));
                    }
                    case "4" -> {
                        System.out.print("Παρακαλώ εισάγεται το IBAN: ");
                        iban = scanner.nextLine().trim();
                ...
                System.out.println("Μη έγκυρη μορφη αριθμού");
